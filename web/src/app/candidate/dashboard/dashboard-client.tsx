...
export default function CandidateDashboardClient({ initialData }: { initialData?: any }) {
  ...
  const { data: dashboardData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [savedJobsRes, appsRes] = await Promise.all([
        apiClient.get('/api/v1/saved-jobs/saved').catch(() => ({ savedJobs: [] } as any)),
        apiClient.get('/api/v1/applications/candidate').catch(() => ({ applications: [] } as any))
      ])
  ...
  useEffect(() => {
    if (dashboardData) {
      setSavedJobs(dashboardData.savedJobs.slice(0, 3).map((item: any) => item.jobs || item))
